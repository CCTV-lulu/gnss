...
function posMiddleUpdate(sta_id,para) {
    var midd;
    var stationMiddle=para.stationMiddle;
...
function middleSaveAll(stationPara) {
    var posave=path.join(__dirname,'/config/posSave.json');
        ...
        if(exist){
            fs.readFile(posave,function (err, data) {
                if(err){
                    ...
                    for(var id in stationPara){
                        if(!data.hasOwnProperty(id)) {
                            data[id]=new posMiddle_create();
                        }
                        data[id].sol=stationPara[id].sol;
                        data[id].count=stationPara[id].count;
                        data[id].mean=stationPara[id].mean;
                        data[id].sigma=stationPara[id].sigma;
                    }
                }
