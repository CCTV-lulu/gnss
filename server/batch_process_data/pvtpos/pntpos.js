    ...
    if (ionoopt==ca.IONOOPT_BRDC && sys== ca.SYS_CMP) {
        if(nav.ion_cmp[sat-1]!=undefined){
            if (cmn.norm(nav.ion_cmp[sat-1].ion,8)<=0.0) {
                ...
                ion[0] = cmn.ionmodel(time, nav.ion_cmp[sat-1].ion, pos, azel);
            }
            else{
                ion[0] = cmn.ionmodel(time, nav.ion_cmp[sat-1].ion, pos, azel);
            }
...
function prange(obs,nav,azel,iter, opt, vare){
    ...
    var i=0,j=1;
    var sys=obs.sys;
    var lam=cmn.satwavelen(obs.sat,obs.sys,nav);
    var tgd=cmn.sattgd(obs.sat,obs.sys,nav);
    vare[0]=0.0;

    if (iter>0) {
        if (cmn.testsnr(0,i,azel[1],obs.SNR[i]*0.25,opt.snrmask)) {
            return 0.0;
        }
        if (opt.ionoopt==ca.IONOOPT_IFLC) {
            if (cmn.testsnr(0,j,azel[1],obs.SNR[j]*0.25,opt.snrmask)) return 0.0;
        }
    }
    gama12=SQR(lam[j])/SQR(lam[i]); /* f1^2/f2^2 */
    ...
    if (opt.ionoopt==ca.IONOOPT_IFLC) { /* dual-frequency */
        ...
        P1=obs.P[i]-tgd[0]*ca.CLIGHT;
        P2=obs.P[j]-tgd[1]*ca.CLIGHT;
...
function rescode(iter, obs, rs,dts, vars, svh,nav, x,NX, opt,v, H, vare,azel,vsat,resp,sst,ns){
    ...
    for (i=0;i<obs.length;i++) {
        ...
        if (ionocorr(obs[i].time,nav,obs[i].sat,obs[i].sys,pos,azt,
                iter>0 ?opt.ionoopt: ca.IONOOPT_BRDC,ion,opt))
            continue;
        /* tropospheric corrections */
        if (tropcorr(obs[i].time,nav,pos,azt,
...
//    if (nv>nx&&vv>chisqr[nv-nx-1]) {
    if (nv>nx && vv>10*cmn.chisqr[nv-nx-1]) {	//放宽检验条件
...
//最小二乘定位解算
function estpos(obs, rs, dts,vars, svh, nav,opt,NX ,sol, vsat,ns,azel,vare,H,v,sst){
    ...
    for (i=0;i<MAXITR;i++) {
        ...
        if (cmn.norm(dx,NX)<1E-4) {
            ...
            //for(j=0;j<nx;j++)H[j]=Ht[j];
            for(j=0;j<nv;j++){v[j]=vt[j];vare[j]=vart[j];sst[j]=sstt[j];};
            ns[0]=nv;
            ns[1]=nsys.length;
            /*for(j=0;j<n;j++){
                sol.azel[2*j]=azel[j*2];
                sol.azel[1+2*j]=azel[1+2*j];
                sol.resp[j]=resp[j];
            }*/
            /* validate solution */
            //console.log(sol.pos);
            stat=valsol_m(azel,vsat,obs.length,opt,vt,nv,4,sol.dop);
            return stat;
        }
    }
    return 0;
...
function raim_fde(obs,rs,dts,vars,svh,nav,opt,NX,sol,vsat,ns,azel,vare,H,v,sst){
    var info,i,j;
    var n,m,k,len;
    var A,S,I,Ht,V,w;
    ...
    if(SSE>opt.threshold_PFD[k]){
        ...
        for(i=0;i<len;i++){
            ...
            H=new Array(NX);
            sst=new Array();
            if(estpos(obs,rs,dts,vars,svh_e,nav,opt,NX,sol,vsat,ns,azel,vare,H,v,sst)){
                rms_e=0;
                for(j=0;j<ns[0];j++){
                    rms_e+=v[j]*v[j];
                }
                rms_e=math.sqrt(rms_e/ns[0]);
                if(rms_e>rms)
                    continue;
                n=ns[0];
                sigma=0;
...
module.exports.pntpos_RAIM=function(obs, nav, opt, sol){
    ...
    var i,stat,vsat,svh,ns=new Array();
    var NX=6;
    var sst=new Array();
    var n=obs.length;
    ...
    if(estpos(obs,rs,dts,vars,svh,nav,opt,NX,sol,vsat,ns,azel,vare,H,v,sst)) {
        /* raim fde */
        stat = raim_fde( obs,rs,dts,vars,svh,nav,opt,NX,sol,vsat,ns,azel,vare,H,v,sst);
        //stat = raim_fde( ns[0],ns[1]+3,H, v,sst, vare, opt, sol);

        /* estimate receiver velocity with doppler */
        //if (stat) estvel(obs, n, rs, dts, nav, opt, sol, azel, vsat);
        sol.stat=stat;
    }
    return stat;
};
