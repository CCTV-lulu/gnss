...
function wavelencreate() {
    ...
    return lam;
}module.exports.wavelencreate=wavelencreate;
function satwavelen(sat,sys,lam){
    var i,frq;
    if (sys==ca.SYS_GLO) {
        if(lam[ca.SYS_GLO][sat-1]==undefined)
            return 0.0;
        return lam[ca.SYS_GLO][sat-1];
    }
    else if (sys==ca.SYS_CMP) {
        return lam[ca.SYS_CMP];
    }
    else if(sys==ca.SYS_GPS){
        return lam[ca.SYS_GPS];
    }
...
function timestruct(tow,millsec) {
    var time=new ca.gtime();
    time.time=tow;
    time.sec=millsec;
    return time;
};module.exports.timestruct=timestruct;
...
function time2string_UTC(time) {
    ...
    return t.getUTCFullYear()+"-"+month +"-"+
        ...
        t.getMilliseconds();
}module.exports.time2string_UTC=time2string_UTC;
function time2string_Local(time) {
    var t=new Date((time.time+time.sec)*1000);
    ...
    return t.getFullYear()+"-"+month +"-"+
        ...
        t.getMilliseconds();
}module.exports.time2string_Local=time2string_Local;
function sysstr(sys) {
    return syscode[sys];
...
function bd2utc(time,utc) {
    var ws=[0,0];
    var te=time,D;
    var tUTC=utc.deltLS+utc.A0utc+utc.A1utc*ws[1];
    time2bdt(time,ws);
    D=math.floor(ws[1]/86400);
    if(timediff(time,bdt2time(utc.WNLSF,(utc.DN+2/3)*86400))>=0 &&
        timediff(time,bdt2time(utc.WNLSF,(utc.DN+5/4)*86400))<=0){
        var W=math.mod(ws[1]-tUTC-43200,86400)+43200;
        te=math.mod(W,86400+utc.deltLSF-utc.deltLS);
    }
    else{
        te=math.mod(ws[1]-tUTC,86400);
    }
    te=bdt2time(ws[0],te+D*86400);
    return te;
}module.exports.bd2utc=bd2utc;
function gps2utc(time,utc) {
    var ws=[0,0];
    var te=time,D;
    time2gpst(time,ws);
    D=math.floor(ws[1]/86400);
    var eff=gpst2time(utc.WNLSF,utc.DN*86400);
    var tUTC=utc.deltLS+utc.A0utc+utc.A1utc*(ws[1]-utc.tot+604800*(ws[0]-utc.WNt));
    if(math.abs(timediff(time,eff))<6*3600){
        var W=math.mod(ws[1]-tUTC-43200,86400)+43200;
        te=math.mod(W,86400+utc.deltLSF-utc.deltLS);
    }
    else{
    ...
    te=gpst2time(ws[0],te+D*86400);
    return te;
}module.exports.gps2utc=gps2utc;
